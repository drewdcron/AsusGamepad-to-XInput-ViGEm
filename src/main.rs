...
fn main() {
    ...
    let client = Client::connect().expect("Failed to connect to ViGEmBus. Is the driver installed?");
    let mut target = Xbox360Wired::new(client, TargetId::XBOX360_WIRED);
    target.plugin().expect("Failed to plug in the virtual controller");
    target.wait_ready().expect("Virtual controller is not ready");
    ...
    let device = api.open(vendor_id, product_id).expect("Failed to hook the ASUS controller!");
        ...
        match device.read(&mut buf) {
            Ok(bytes_read) if bytes_read == 9 => {
                ...
                if buf != last_buf {
                    ...
                    if dpad_up { raw_buttons |= 0x0001; }
                    if dpad_down { raw_buttons |= 0x0002; }
                    if dpad_left { raw_buttons |= 0x0004; }
                    if dpad_right { raw_buttons |= 0x0008; }
                    if btn_start { raw_buttons |= 0x0010; }
                    if btn_back { raw_buttons |= 0x0020; }
                    if stick_l_click { raw_buttons |= 0x0040; }
                    if stick_r_click { raw_buttons |= 0x0080; }
                    if bumper_l { raw_buttons |= 0x0100; }
                    if bumper_r { raw_buttons |= 0x0200; }
                    if btn_a { raw_buttons |= 0x1000; }
                    if btn_b { raw_buttons |= 0x2000; }
                    if btn_x { raw_buttons |= 0x4000; }
                    if btn_y { raw_buttons |= 0x8000; }
